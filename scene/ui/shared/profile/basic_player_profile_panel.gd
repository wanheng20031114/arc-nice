extends CanvasLayer
class_name BasicPlayerProfilePanel
...
func _on_inventory_item_use_requested(slot_index: int) -> void:
	...
	if item == null:
		inventory_view.refresh()
		return
	# This neutral profile does not own building placement. Mode wrappers that
	# support placement provide their own implementation.
