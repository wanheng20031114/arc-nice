extends Enemy
class_name FireRangedEnemy

const WORLD_COLLISION_MASK := 1
const FireConfig := preload("res://resources/config/fire_ranged_enemy_config.gd")
...
func _try_fire_ranged_projectile() -> bool:
	...
	var projectile := fire_config.projectile_scene.instantiate() as EnemyFireProjectile
	if projectile == null:
		push_warning("Fire projectile scene must instantiate EnemyFireProjectile.")
		return false
