...
func _unhandled_input(event: InputEvent) -> void:
	...
	if special_game_overlay.is_open():
		if special_game_overlay.handle_input(event):
...
func _advance_dialogue() -> void:
	...
	if special_game_dialogue_active:
...
func apply_special_game_card_revealed(result: Dictionary) -> void:
	if (
		...
		or int(result.get("session_revision", 0))
...
func _request_special_game_start() -> void:
	...
	_arm_authoritative_request_timeout(
		AuthoritativeRequestKind.SPECIAL_START
	)
...
func _on_special_game_card_reveal_requested(card_index: int) -> void:
	if special_game_session_revision <= 0:
		return
	_arm_authoritative_request_timeout(
		AuthoritativeRequestKind.SPECIAL_REVEAL
	)
	var current_scene := get_tree().current_scene
	if (
		current_scene == null
...
func _on_special_game_finish_requested() -> void:
	if special_game_session_revision <= 0:
		return
	_arm_authoritative_request_timeout(
		AuthoritativeRequestKind.SPECIAL_FINISH
	)
	var current_scene := get_tree().current_scene
	if (
		current_scene == null
...
func _on_observed_player_died(player_instance: Player) -> void:
	# 洛茜可能依次与多个联机玩家交谈。只响应当前对话或当前牌局玩家的
	# 死亡，不能让早先交互过的玩家在远处死亡时关掉别人的界面。
	if (
		player_instance != active_player
		and player_instance != special_game_player
		and player_instance != special_game_request_player
	):
		return
	_clear_authoritative_request_wait(true)
	special_game_dialogue_active = false
...
func _build_dialogue_lines(player: Player) -> Array:
	if special_game_resume_pending and player == special_game_player:
		special_game_dialogue_active = true
		return SPECIAL_GAME_RESUME_DIALOGUE_LINES.duplicate()
	if _player_has_special_game_ticket(player):
		special_game_dialogue_active = true
...
func _on_interaction_area_body_exited(body: Node2D) -> void:
	...
	if player != active_player:
		return
	if player == special_game_player and special_game_overlay.is_open():
		dialogue_bubble.hide_bubble()
		choice_visible = false
		choice_overlay.hide_choices()
		return
	if player == special_game_request_player:
		_clear_authoritative_request_wait(true)
		_release_special_game_request_player()
...
func _clear_authoritative_request_wait(clear_refresh_pending: bool = false) -> void:
	...
	if clear_refresh_pending and special_game_overlay.is_open():
		special_game_overlay.set_pending(false)
...
func _on_authoritative_request_timeout() -> void:
	...
	elif expired_kind == AuthoritativeRequestKind.SPECIAL_START:
		if (
			special_game_request_player != null
			and not special_game_request_player.is_dead
		):
