...
var boss_coordinator: StandardBossCoordinator = null
var pause_music_request: Callable
...
func bind_standard_dependencies(
	coordinator: StandardBossCoordinator,
	pause_music: Callable
) -> void:
	boss_coordinator = coordinator
	pause_music_request = pause_music
...
func is_terminal_combat_state() -> bool:
	var wave_runtime := combat_runtime as WaveCombatRuntimeBase
	return (
		wave_runtime != null
		and wave_runtime.wave_state in [
...
func pause_background_music() -> void:
	if pause_music_request.is_valid():
		pause_music_request.call()
...
func get_enrage_sniper_config() -> EnemyConfig:
	return (
		boss_coordinator.get_enrage_sniper_config()
...
func get_skill2_target_player(from_position: Vector2) -> Player:
	return (
		boss_coordinator.get_skill2_target_player(from_position)
		if boss_coordinator != null
		else null
	)
