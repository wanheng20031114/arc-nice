...
var local_upgrade_authority_enabled := true
...
func configure_local_upgrade_authority(enabled: bool) -> void:
	local_upgrade_authority_enabled = enabled
...
func _on_upgrade_requested(stat_type: int) -> void:
	...
	if multiplayer_requests_enabled:
		...
		return
	if not local_upgrade_authority_enabled:
		return
...
func _on_inventory_item_use_requested(slot_index: int) -> void:
	...
	if item == null:
		inventory_view.refresh()
		return
	# Standard mode never owns building placement. Preserve the previous no-op
	# for a building item without probing the current scene for tower features.
