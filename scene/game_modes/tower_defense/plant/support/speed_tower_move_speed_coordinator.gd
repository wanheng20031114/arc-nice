extends Node
class_name SpeedTowerMoveSpeedCoordinator

@export var plant_system: PlantSystem
@export var player_roster_coordinator: TowerDefensePlayerRosterCoordinator

var active_sources: Dictionary[SpeedTower, bool] = {}
var pending_sources: Dictionary[SpeedTower, Callable] = {}
var total_move_speed_bonus := 0.0


func _ready() -> void:
	if plant_system == null or player_roster_coordinator == null:
		push_error("SpeedTowerMoveSpeedCoordinator requires PlantSystem and player roster.")
		return
...
func _on_plant_placed(plant: PlantDefense) -> void:
	var source := plant as SpeedTower
...
func _on_source_construction_finished(source: SpeedTower) -> void:
...
func _activate_source(source: SpeedTower) -> void:
	if (
		source == null
		or not is_instance_valid(source)
		or source.speed_tower_config == null
...
func _disconnect_pending_source(source: SpeedTower) -> void:
...
func _refresh_total_bonus() -> void:
	var next_total := 0.0
	var stale_sources: Array[SpeedTower] = []
	for source in active_sources:
		if (
			...
			continue
		next_total += source.get_player_move_speed_bonus()
	for source in stale_sources:
		active_sources.erase(source)
	total_move_speed_bonus = maxf(next_total, 0.0)
...
func _apply_bonus_to_player(player: Player) -> void:
	if player == null or not is_instance_valid(player):
		return
	player.set_tower_defense_speed_tower_bonus(total_move_speed_bonus)
