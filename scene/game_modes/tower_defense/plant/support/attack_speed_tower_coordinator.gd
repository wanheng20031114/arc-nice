extends Node
class_name AttackSpeedTowerCoordinator

@export var plant_system: PlantSystem
@export var player_roster_coordinator: TowerDefensePlayerRosterCoordinator

var active_sources: Dictionary[AttackSpeedTower, bool] = {}
var pending_sources: Dictionary[AttackSpeedTower, Callable] = {}
var total_bonus_ratio := 0.0


func _ready() -> void:
	if plant_system == null or player_roster_coordinator == null:
		push_error("AttackSpeedTowerCoordinator requires PlantSystem and player roster.")
		return
...
func _on_plant_placed(plant: PlantDefense) -> void:
	var source := plant as AttackSpeedTower
...
func _on_source_construction_finished(source: AttackSpeedTower) -> void:
...
func _activate_source(source: AttackSpeedTower) -> void:
	if (
		source == null
		or not is_instance_valid(source)
		or source.attack_speed_tower_config == null
...
func _disconnect_pending_source(source: AttackSpeedTower) -> void:
...
func _refresh_total_bonus() -> void:
	var next_total := 0.0
	var stale_sources: Array[AttackSpeedTower] = []
	for source in active_sources:
		if (
			...
			continue
		next_total += source.get_player_attack_speed_bonus_ratio()
...
func _apply_bonus_to_player(player: Player) -> void:
	if player == null or not is_instance_valid(player):
		return
	player.set_tower_defense_attack_speed_tower_bonus_ratio(total_bonus_ratio)
