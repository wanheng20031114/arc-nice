...
func bind_runtime(runtime_instance: CombatRuntimeBase) -> void:
	...
	game = runtime_instance as TowerDefenseGame
...
func get_home_objective_target(from_position: Vector2) -> Node2D:
	return game.get_linglan_home_objective_target(from_position) if game != null else null


func spawn_random_slime(spawn_position: Vector2) -> void:
	if game != null:
		game.spawn_linglan_random_slime(spawn_position)
