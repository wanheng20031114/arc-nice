...
4. 单样例成立后再生成动画大图。
   ...
   生图 prompt 可以使用中文，也可以使用英文；本项目角色名、动作名、道具名用中文更清楚时就直接用中文。Prompt 要明确网格行列、稳定镜头、稳定主体大小、主体中心/脚底锚点一致、每格完整角色、每格安全框、纯平背景、背景无阴影、格子之间不重叠。帧数和行列由当前动画需求决定，不存在通用固定帧数或固定网格；示例里的 `[列数]列 x [行数]行` 必须在实际任务中替换成当次需要的数量。行语义要直接写清楚，例如 `idle`、`move`、`attack`、`death`，或者中文 `站立`、`横向移动`、`举杖攻击`、`死亡`。
...
[列数]列 x [行数]行，每格一个完整帧，透明 alpha 或纯平 [背景色] 背景。[列数]、[行数]、总帧数由当前动画动作决定。
第1行：[动作名]；第2行：[动作名]；...
