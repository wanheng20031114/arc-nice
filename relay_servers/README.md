...
当前网络基线为协议 v87、8 个 ENet 通道。v87 将 WaterCollector 固定为
连续循环采集，并从产线面板隐藏单次/无限循环 mode；v86 及更旧
客户端缺少该统一运行时契约，不能与 v87 房间安全混联。v86 在正式
塔防中新增 Client 到 Host 的“销毁最近建筑”可靠请求；Host 必须根据
