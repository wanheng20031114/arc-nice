...
# 每次全新部署生成独立高熵 HMAC secret；值只写入权限受限的 .env，绝不打印。
umask 077
ACQUISITION_CAPABILITY_HMAC_SECRET="$(openssl rand -hex 48)"
if [ "${#ACQUISITION_CAPABILITY_HMAC_SECRET}" -ne 96 ]; then
    echo "  无法生成 acquisition HMAC secret，部署终止。" >&2
    exit 1
fi
cat > "$ENV_FILE" <<EOF
PUBLIC_IP=$PUBLIC_IP
...
ACQUISITION_CAPABILITY_HMAC_SECRET=$ACQUISITION_CAPABILITY_HMAC_SECRET
...
ROOM_IDLE_TIMEOUT_SECONDS=180
RELAY_STARTUP_IDLE_TIMEOUT_SECONDS=120
RELAY_EMPTY_IDLE_TIMEOUT_SECONDS=120
GAME_MAX_DURATION_SECONDS=36000
CLEANUP_INTERVAL_SECONDS=30
RELAY_STARTUP_GRACE_SECONDS=5
RELAY_ADMISSION_TICKET_TTL_SECONDS=60
RELAY_ADMISSION_REFRESH_BURST=3
RELAY_ADMISSION_REFRESH_WINDOW_SECONDS=5
EOF
chmod 600 "$ENV_FILE"
unset ACQUISITION_CAPABILITY_HMAC_SECRET
