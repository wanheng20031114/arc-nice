...
func _test_codex_and_fate() -> void:
	...
	_expect(
		...
		"图鉴计数必须为61/49/11/1/10且精英忍者只注册一次。"
	)
...
func _test_protocol_and_no_new_attack_resources() -> void:
	_expect(NET_CONSTANTS.PROTOCOL_VERSION == 54, "精英忍者必须使用协议v54。")
	_expect(
...
func _test_texture_counts() -> void:
	...
	_expect(
		all_enemy_pngs == 85 and mechanical_pngs == 20,
		"敌人纹理必须为85张，机械生命必须为20张；实际%d/%d。"
