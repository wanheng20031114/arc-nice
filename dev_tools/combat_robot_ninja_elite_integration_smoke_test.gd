...
func _test_codex_and_fate() -> void:
	...
	_expect(
		...
		"图鉴计数必须为62/50/11/1、机械生命10、人工造物3，且精英忍者只注册一次。"
	)
...
func _test_protocol_and_no_new_attack_resources() -> void:
	_expect(NET_CONSTANTS.PROTOCOL_VERSION == 58, "精英忍者资源合同必须保留在协议v58。")
	_expect(
...
func _test_texture_counts() -> void:
	...
	_expect(
		all_enemy_pngs == 86 and artificial_pngs == 3 and mechanical_pngs == 20,
