...
const STANDARD_SINGLEPLAYER_SLIME := preload(
...
func _generate_campaign(definition: Dictionary, failures: PackedStringArray) -> void:
	...
	for wave_number in range(1, 13):
		...
		if (
			...
			_configure_standard_singleplayer_slime_wave(
				campaign_wave
			)
