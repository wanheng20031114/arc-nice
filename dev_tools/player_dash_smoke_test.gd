...
func _test_player_scene(player_scene: PackedScene, label: String) -> void:
	...
	_expect(is_equal_approx(player.dash_cooldown, 5.0), "%s default dash cooldown must be 5 seconds." % label)
	player.dash_distance = 25.0
	...
	_expect(
		ready_material != null and is_equal_approx(float(ready_material.get_shader_parameter(READY_STRENGTH_PARAMETER)), 1.0),
		"%s ready glow must be active before the first dash." % label
	)
	_expect(
	...
	_expect(
		absf(displacement.length() - 25.0) <= 0.75,
		"%s unobstructed dash must travel about 25 px (actual %.3f)." % [label, displacement.length()]
	)
	_expect(
	...
	_expect(
		ready_material != null and is_equal_approx(float(ready_material.get_shader_parameter(READY_STRENGTH_PARAMETER)), 1.0),
		"%s ready glow must return when cooldown completes." % label
	)
