...
const NAVIGATION_SCENE_PATH := (
	"res://scene/game_modes/rogue/combat/rogue_combat_game_04_navigation.tscn"
)
const WORLD_RECT := Rect2i(0, 0, 26, 18)
const CELL_SIZE := Vector2i(16, 16)
## 以几乎完整的格子足迹采样，而不是只检查格心。这样 authored 墙体
## 即使只侵入某格数个像素，该格也不会被小体型敌人错误视为可走；
## 0.02px 的总间隙则避免把刚好贴住格边的下一行一并判为阻挡。
const CELL_PROBE_SIZE := Vector2(15.98, 15.98)
...
func _generate_navigation() -> bool:
	...
	cell_probe.size = CELL_PROBE_SIZE
	var blocked_count := 0
	for y in range(WORLD_RECT.position.y, WORLD_RECT.end.y):
		for x in range(WORLD_RECT.position.x, WORLD_RECT.end.x):
			...
			var blocked := _shape_hits_wall(center, cell_probe, wall)
	...
	if used_rect != WORLD_RECT or used_cell_count != 468:
		push_error(
	...
	if pack_error != OK:
		push_error("无法打包地下水道导航场景：%s" % error_string(pack_error))
		navigation_layer.free()
		game.free()
		return false
	...
	game.free()
	if save_error != OK:
		push_error("无法保存地下水道导航场景：%s" % error_string(save_error))
		return false
	print(
		"ROGUE_COMBAT_GAME_04_NAVIGATION_GENERATED shapes=%d blocked=%d open=%d"
