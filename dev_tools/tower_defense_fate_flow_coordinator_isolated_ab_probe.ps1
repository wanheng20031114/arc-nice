...
$ExpectedBaseline = "f241fedb5f20959d9bd158980a7f0e2cab3b44aa"
...
function Invoke-FateFlowProbe {
    ...
    try {
        ...
        $Process.WaitForExit()
        $CombinedOutput = (
            $StandardOutputTask.Result + "`n" + $StandardErrorTask.Result
        )
        if ($TimedOut) {
        ...
        if (
            ...
            throw (
                "$Label probe failed with exit code $($Process.ExitCode):" +
                "`n$CombinedOutput"
            )
        }
        $HashMatch = [regex]::Match(
            $CombinedOutput,
            "TOWER_FATE_FLOW_AB_HASH=([0-9a-f]{64})"
        )
        ...
        return [pscustomobject]@{
            ...
            FixedRefCountedExitNoise = (
                $CombinedOutput -match "ObjectDB instances leaked at exit"
            )
        }
    }
    finally {
...
for ($Round = 1; $Round -le $Rounds; $Round++) {
    if ($Round % 2 -eq 1) {
        $BaselineResults += Invoke-FateFlowProbe "baseline-$Round" $BaselinePath
        $CurrentResults += Invoke-FateFlowProbe "current-$Round" $CurrentPath
    }
    else {
        $CurrentResults += Invoke-FateFlowProbe "current-$Round" $CurrentPath
        $BaselineResults += Invoke-FateFlowProbe "baseline-$Round" $BaselinePath
...
$AllHashes = @(
    $BaselineResults.Hash + $CurrentResults.Hash | Select-Object -Unique
)
if ($AllHashes.Count -ne 1) {
    throw "FateFlow behavior trace mismatch: $($AllHashes -join ', ')"
}
...
$MemoryP50Passed = (
    $CurrentMemoryP50 -le ($BaselineMemoryP50 + $MemoryP50Budget)
)
$MemoryP95Passed = (
    $CurrentMemoryP95 -le ($BaselineMemoryP95 + $MemoryP95Budget)
)
$ExitNoiseSymmetric = (
    @(
        $BaselineResults.FixedRefCountedExitNoise | Select-Object -Unique
    ).Count -eq 1 -and
    @(
        $CurrentResults.FixedRefCountedExitNoise | Select-Object -Unique
    ).Count -eq 1 -and
...
if (
    ...
    throw "FateFlow A/B performance or fixed-noise gate failed."
}
