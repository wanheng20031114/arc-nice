...
func _run() -> void:
	...
	_test_output_weight_boundaries()
...
func _test_config_and_scene(excavator: Excavator) -> void:
	...
	_expect(
		...
		"挖土装置场景必须只挂载自动选择的挖掘循环配方。"
	)


func _test_output_weight_boundaries() -> void:
	var category_counts: Dictionary[StringName, int] = {}
	for roll in range(100):
		var category := Excavator.get_output_category_for_roll(roll)
		category_counts[category] = category_counts.get(category, 0) + 1
	_expect(
		category_counts.get(Excavator.OUTPUT_DIRT, 0) == 50
		and category_counts.get(Excavator.OUTPUT_PLANK, 0) == 25
		and category_counts.get(Excavator.OUTPUT_HEALTH_POTION, 0) == 20
		and category_counts.get(Excavator.OUTPUT_COMMON_COLLECTIBLE, 0) == 4
		and category_counts.get(Excavator.OUTPUT_NON_COMMON_COLLECTIBLE, 0) == 1,
		"0..99权重必须严格分配为50土块、25木板、20生命药水、4普通收藏品和1非普通收藏品。"
	)
	_expect(
		Excavator.get_output_category_for_roll(0) == Excavator.OUTPUT_DIRT
		and Excavator.get_output_category_for_roll(49) == Excavator.OUTPUT_DIRT
		and Excavator.get_output_category_for_roll(50) == Excavator.OUTPUT_PLANK
		and Excavator.get_output_category_for_roll(74) == Excavator.OUTPUT_PLANK
		and Excavator.get_output_category_for_roll(75)
		== Excavator.OUTPUT_HEALTH_POTION
		and Excavator.get_output_category_for_roll(94)
		== Excavator.OUTPUT_HEALTH_POTION
		and Excavator.get_output_category_for_roll(95)
		== Excavator.OUTPUT_COMMON_COLLECTIBLE
		and Excavator.get_output_category_for_roll(98)
		== Excavator.OUTPUT_COMMON_COLLECTIBLE
		and Excavator.get_output_category_for_roll(99)
		== Excavator.OUTPUT_NON_COMMON_COLLECTIBLE
		and Excavator.get_output_category_for_roll(-1) == &""
		and Excavator.get_output_category_for_roll(100) == &"",
		"挖土装置随机表必须在49/50、74/75、94/95、98/99处准确切换并拒绝越界roll。"
	)
...
func _test_local_output_slot_and_runtime_state(
	...
	_expect(
		...
		"产物未领取时即使再过120秒也必须保持单件堵塞，不得继续计时或覆盖产物。"
	)
	...
	_expect(
		...
		and excavator.completion_wait_reason == &""
		and run_state.get_inventory_item_total(output_item) == inventory_before + 1
		and run_state.get_inventory_revision() == inventory_revision_before + 1,
		"玩家领取后必须原子清空产物格、解除堵塞并把恰好1件物品加入背包。"
	)
	excavator.advance_shared_production_tick(1.0)
	_expect(
	...
	_expect(
		...
		"Host必须把多人collect_output原子提交到请求玩家背包，再同步清槽revision与目标peer事件。"
	)
...
func _test_runtime_state_and_collect_protocol(
	...
	_expect(
		...
		and int(authoritative_state.get("buffered_output_count", 0)) == 1
...
func _is_valid_excavator_output(item: PickupConfig) -> bool:
	return (
		item == DIRT_BLOCK
		or item == PLANK
		or item == HEALTH_POTION
		or (item != null and CollectibleRegistry.is_collectible_path(item.resource_path))
	)
