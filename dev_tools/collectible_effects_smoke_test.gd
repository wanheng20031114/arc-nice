...
func _test_admin_doll_free_upgrade() -> void:
	...
	_expect(player.has_skill1(), "Players must start with skill1 before admin doll upgrades.")
	_expect(player.get_skill1_upgrade_cost() == 200, "The first skill1 upgrade must normally cost 200 xirang.")
	_expect(player.try_upgrade_skill1(player.has_collectible_effect(PickupConfig.COLLECTIBLE_EFFECT_ADMIN_DOLL)), "Admin doll must make skill1 upgrades free.")
	_expect(player.current_xirang == 0, "Admin doll upgrade must not spend xirang.")
	_expect(player.skill1_upgrade_level == 1, "Admin doll must apply the first skill1 upgrade.")
