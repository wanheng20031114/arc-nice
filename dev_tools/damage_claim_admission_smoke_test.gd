...
## Security contract for player damage ingress. Protocol v22 retains a
...
func _test_player_hit_claim_lane_is_disabled() -> void:
	...
	_expect(
		...
		"The protocol-v22 compatibility RPC must remain exactly five fields. actual=%s"
		% signature
	)
...
func _test_enemy_hit_claim_lane_is_disabled() -> void:
	...
	_expect(
		signature == expected_signature,
		"The protocol-v22 enemy-hit compatibility RPC must retain its exact shape. actual=%s"
		% signature
	)
	var request_body := _get_function_body(
		source,
