...
func _test_boss_entry_resource() -> void:
	...
	_expect(LINGLAN_BOSS_ENTRY.intro_vfx_scene_path == "res://scene/boss/linglan/linglan_boss_intro_vfx.tscn", "Linglan boss entry must point to the configured intro VFX scene.")
	_expect(LINGLAN_BOSS_ENTRY.boss_hud_scene_path == "res://scene/boss/linglan/boss_health_hud.tscn", "Linglan boss entry must point to the configured boss HUD scene.")
	...
	_expect(is_equal_approx(LINGLAN_CONFIG.move_speed, 20.0), "Linglan boss move speed must be configured to 20.")
...
func _test_boss_scene_contract() -> void:
	...
	_expect(linglan.current_health == max_health - 95, "Linglan health did not decrease after physical defense.")
...
func _test_boss_hud_binding() -> void:
	...
	await process_frame
	_expect(health_bar != null and health_bar.value == float(max_health - 470), "Boss HUD must track Linglan damage after physical defense.")
