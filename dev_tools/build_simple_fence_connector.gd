...
func _init() -> void:
	...
	var connector := _extract_symmetric_connector(atlas)
...
func _extract_symmetric_connector(atlas: Image) -> Image:
	...
	for y in range(CONNECTOR_SIZE):
		var inset: int = EDGE_INSETS[y]
		var source_y: int = (
			y
			if y < SOURCE_QUADRANT_SIZE
			else CONNECTOR_SIZE - 1 - y
		)
		for x in range(inset, CONNECTOR_SIZE - inset):
			var source_x: int = (
...
func _validate_connector(connector: Image) -> String:
	if connector == null or connector.get_size() != Vector2i(CONNECTOR_SIZE, CONNECTOR_SIZE):
		return "Simple fence connector must be exactly 10x10."
	var visible_pixels := 0
	for y in range(CONNECTOR_SIZE):
		for x in range(CONNECTOR_SIZE):
			var color := connector.get_pixel(x, y)
			if color.a != 0.0 and color.a != 1.0:
				return "Simple fence connector must use binary alpha."
			if color.a > 0.0:
				visible_pixels += 1
			if color != connector.get_pixel(CONNECTOR_SIZE - 1 - x, y):
				return "Simple fence connector must be horizontally symmetric."
			if color != connector.get_pixel(x, CONNECTOR_SIZE - 1 - y):
				return "Simple fence connector must be vertically symmetric."
	if visible_pixels != 88:
