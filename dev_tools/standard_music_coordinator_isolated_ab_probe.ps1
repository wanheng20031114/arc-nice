...
if ($RoundCount -lt 5) {
    throw "Standard Music isolated A/B requires at least five rounds."
}
if ($ArmTimeoutSeconds -lt 1) {
    throw "Standard Music isolated A/B arm timeout must be positive."
}

function Invoke-StandardMusicArm {
    param(
    ...
    try {
        ...
        $startInfo.Arguments = (
            "--headless --path `"{0}`" --script res://dev_tools/standard_music_coordinator_ab_probe.gd -- --arm={1}" `
            -f $projectRoot, $Arm
        )
        ...
        if (-not $process.Start()) {
            throw "Unable to start Standard Music $Arm arm."
        }
        ...
        while (-not $process.HasExited) {
            ...
            if ([DateTime]::UtcNow -ge $deadline) {
                $process.Kill()
                $process.WaitForExit()
                throw "Standard Music $Arm arm exceeded $ArmTimeoutSeconds seconds."
            }
        ...
        if ($exitCode -ne 0) {
            throw "Standard Music $Arm arm failed with exit $exitCode`: $stdout $stderr"
        }
        $markerPattern = (
            "STANDARD_MUSIC_COORDINATOR_AB_ARM_OK arm=(legacy|extracted) p50_usec=(\d+) p95_usec=(\d+) trajectory_hash=(-?\d+)"
        )
        ...
        if ($badCount -ne 0) {
            throw "Standard Music $Arm arm emitted $badCount error markers: $stdout $stderr"
        }
        if ($markerMatches.Count -ne 1) {
            throw "Standard Music $Arm arm emitted $($markerMatches.Count) success markers: $stdout $stderr"
        }
        $match = $markerMatches[0]
        if ($match.Groups[1].Value -ne $Arm) {
            throw "Standard Music arm marker mismatch: requested=$Arm actual=$($match.Groups[1].Value)"
        }
...
for ($round = 1; $round -le $RoundCount; $round++) {
    if ($round % 2 -eq 1) {
        $legacy = Invoke-StandardMusicArm -Arm legacy
        $extracted = Invoke-StandardMusicArm -Arm extracted
    } else {
        $extracted = Invoke-StandardMusicArm -Arm extracted
        $legacy = Invoke-StandardMusicArm -Arm legacy
    }
    ...
    Write-Output (
        "STANDARD_MUSIC_COORDINATOR_ISOLATED_AB_SAMPLE round={0} legacy_p50_usec={1} extracted_p50_usec={2} legacy_p95_usec={3} extracted_p95_usec={4} legacy_peak_bytes={5} extracted_peak_bytes={6} trajectory_hash={7} marker=1 bad=0 exit=0" `
        -f $round,
        $legacy.P50Usec,
        $extracted.P50Usec,
        $legacy.P95Usec,
...
Write-Output (
    "STANDARD_MUSIC_COORDINATOR_ISOLATED_AB_PROBE_OK rounds={0} legacy_p50_usec={1} extracted_p50_usec={2} legacy_p95_usec={3} extracted_p95_usec={4} p50_limit_usec={5} p95_limit_usec={6} legacy_p50_bytes={7} extracted_p50_bytes={8} legacy_p95_bytes={9} extracted_p95_bytes={10} p50_limit_bytes={11} p95_limit_bytes={12}" `
    -f $RoundCount,
    $legacyP50Usec,
    $extractedP50Usec,
    $legacyP95Usec,
