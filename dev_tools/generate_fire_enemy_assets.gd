...
func _init() -> void:
	...
	if source == null or source.is_empty():
		push_error("Unable to load source enemy texture: %s" % SOURCE_TEXTURE_PATH)
		quit(1)
		return

	var enemy_sheet := _build_enemy_sheet(source)
	...
	if enemy_error != OK or projectile_error != OK or audio_error != OK:
		push_error(
			"Asset generation failed: enemy=%s projectile=%s audio=%s"
			% [enemy_error, projectile_error, audio_error]
		)
		quit(1)
		return

	print("Generated fire enemy assets.")
	quit()


func _build_enemy_sheet(source: Image) -> Image:
...
func _recolor_fire_enemy(image: Image) -> void:
