...
const EXPECTED_CATEGORY_BY_ID := {
	...
	&"attack_speed_tower": PlantDefenseConfig.BuildingCategory.SUPPORT_TOWER,
...
const EXPECTED_SURFACE_BY_ID := {
	...
	&"attack_speed_tower": PlantDefenseConfig.PlacementSurface.GRASS_ONLY,
...
func _test_registry_semantics() -> void:
	...
	_expect(configs.size() == 19, "正式建筑注册表必须恰好包含19项。")
	_expect(
	...
	_expect(
		...
		"七类建筑数量必须固定为4/5/6/1/1/1/1。actual=%s" % [category_counts]
	)
	...
	var attack_speed_tower := (
		PlantDefenseRegistry.get_config(&"attack_speed_tower")
		as AttackSpeedTowerConfig
	)
	_expect(
		attack_speed_tower != null
		and attack_speed_tower.max_health == 2400
		and attack_speed_tower.physical_defense == 5
		and attack_speed_tower.magic_defense == 0
		and attack_speed_tower.footprint_size == Vector2i(2, 2)
		and attack_speed_tower.supports_multiplayer
		and is_equal_approx(attack_speed_tower.attack_speed_bonus_ratio, 0.03),
		"攻速强化塔必须声明2×2占地、2400生命、5物防、0法防与每塔3%攻击速度加成。"
	)
...
func _test_building_item_and_acquisition_closure() -> void:
	_expect(
		...
		"建筑物品必须与19个plant_id形成一一对应，且每项至少有一条获取路线。"
	)
	_expect(
		BuildingItemRegistry.get_all_items().size() == 19,
		"建筑物品Registry必须恰好公开19项。"
	)
	_expect(
	...
	_expect(
		attack_speed_recipe != null
		and attack_speed_recipe.display_name == "攻速强化塔组装"
		and attack_speed_recipe.input_items == [PLANK, SAPLING]
		and attack_speed_recipe.input_amounts == [10, 2]
		and attack_speed_recipe.output_items
