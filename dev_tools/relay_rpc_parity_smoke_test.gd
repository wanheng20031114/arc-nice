...
func _run() -> void:
	...
	_expect(
		NetConstants.PROTOCOL_VERSION == 83,
		"协议v83必须保留 v79 T 目录付费、v78 Route 升级事务及 v77 内容摘要和身份合同。"
	)
...
func _test_registration_protocol_handshake_source() -> void:
	...
	_expect(
		...
		"Protocol v83 hosts must accept exactly v83 and reject v82."
	)
	net_manager.free()
...
func _test_relay_channel_count() -> void:
	...
	_expect(
		...
		and relay_source.contains("const PROTOCOL_VERSION := 83")
		and relay_source.contains("--max-clients=")
		and relay_source.contains("create_server(_port, _max_clients, CHANNEL_COUNT)"),
		(
			"Relay server must declare v83, accept the room capacity, and provision "
