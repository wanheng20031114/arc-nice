...
func _run() -> void:
	...
	_test_gameplay_v13_transaction_contract(main_rpcs)
	...
	_expect(
		...
		"Reliable terminal feedback on the simple-crafting baseline requires protocol version 13."
	)
	_expect(NetConstants.CHANNEL_COUNT == 8, "Protocol v13 must provision eight ENet channels.")
...
func _test_relay_channel_count() -> void:
	...
	_expect(
		...
		"Relay server must provision the same eight ENet channels as protocol v13 clients."
	)


func _test_gameplay_v13_transaction_contract(rpcs: Dictionary) -> void:
	...
	_expect(
		...
		"Protocol v13 terrain repair must use 96-cell chunks, preserve EMPTY=-1, and rate-limit repair requests."
	)
	for signature_fragment in [
