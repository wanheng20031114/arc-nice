...
func _run() -> void:
	...
	_expect(
		...
		"协议v79必须保留 v78 Route 升级事务、进度账本及 v77 内容摘要和身份合同。"
	)
...
func _test_registration_protocol_handshake_source() -> void:
	...
	_expect(
		...
		"Protocol v79 hosts must accept exactly v79 and reject v78."
	)
	net_manager.free()
...
func _test_relay_channel_count() -> void:
	...
	_expect(
		...
		and relay_source.contains("const PROTOCOL_VERSION := 79")
		and relay_source.contains("--max-clients=")
		and relay_source.contains("create_server(_port, _max_clients, CHANNEL_COUNT)"),
		(
			"Relay server must declare v79, accept the room capacity, and provision "
