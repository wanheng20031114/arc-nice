...
func _run() -> void:
	...
	_test_gameplay_v16_transaction_contract(main_rpcs)
	...
	_expect(
		...
		"Enemy locomotion snapshots require protocol version 16."
	)
...
func _test_relay_channel_count() -> void:
	...
	_expect(
		...
		"Relay server must provision the same eight ENet channels as protocol v16 clients."
	)


func _test_gameplay_v16_transaction_contract(rpcs: Dictionary) -> void:
	...
	_expect(
		...
		"Protocol v16 terrain repair must use 96-cell chunks, preserve EMPTY=-1, and rate-limit repair requests."
	)
	_expect(
		...
		"Protocol v16 research commands must use schema2 and resolve a Host-owned research whitelist."
	)
	for signature_fragment in [
