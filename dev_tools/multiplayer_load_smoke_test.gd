...
func _test_net_manager_protocol_version_gate() -> void:
	...
	net_manager.disconnect_from_game()
	_expect(NetConstants.PROTOCOL_VERSION == 92, "The multiplayer protocol version must be 92.")
	_expect(
		NetConstants.CH_MEMBERSHIP == 8
		and NetConstants.ENET_MAX_CHANNEL == 8
		and NetConstants.CHANNEL_COUNT == 9,
		"Protocol v92 must expose logical CH0..CH8 with ENet max channel 8."
	)
	_expect(
	...
	for incompatible_version: int in [
		...
		_expect(
			...
			"Protocol v92 must reject incompatible version %d."
			% incompatible_version
		)
