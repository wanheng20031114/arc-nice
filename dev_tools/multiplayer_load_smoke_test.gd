...
func _test_net_manager_protocol_version_gate() -> void:
	...
	net_manager.disconnect_from_game()
	_expect(NetConstants.PROTOCOL_VERSION == 69, "The multiplayer protocol version must be 69.")
	_expect(NetConstants.CHANNEL_COUNT == 8, "Protocol v69 must retain eight ENet channels.")
	_expect(
	...
	_expect(
		not bool(net_manager.call("_is_protocol_version_compatible", 67))
		...
		"Protocol v69 must reject v68 and all older or unversioned clients."
	)
