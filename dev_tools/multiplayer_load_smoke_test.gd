...
func _test_net_manager_protocol_version_gate() -> void:
	...
	net_manager.disconnect_from_game()
	_expect(NetConstants.PROTOCOL_VERSION == 13, "The multiplayer protocol version must be 13.")
	...
	_expect(
		not bool(net_manager.call("_is_protocol_version_compatible", 12))
		...
		"NetManager must reject protocol versions 12/11/10/9/8/7/6/5/4/3/2 and registrations with no version."
	)
