...
func _test_net_manager_protocol_version_gate() -> void:
	...
	net_manager.disconnect_from_game()
	_expect(NetConstants.PROTOCOL_VERSION == 47, "The multiplayer protocol version must be 47.")
	...
	_expect(
		not bool(net_manager.call("_is_protocol_version_compatible", 46))
		...
		"Protocol v47 must reject v46 and all older or unversioned clients."
	)
