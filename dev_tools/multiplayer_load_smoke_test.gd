...
func _test_net_manager_protocol_version_gate() -> void:
	...
	net_manager.disconnect_from_game()
	_expect(NetConstants.PROTOCOL_VERSION == 61, "The multiplayer protocol version must be 61.")
	...
	_expect(
		not bool(net_manager.call("_is_protocol_version_compatible", 60))
		...
		"Protocol v61 must reject v60 and all older or unversioned clients."
	)
