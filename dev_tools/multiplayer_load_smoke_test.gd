...
func _test_net_manager_protocol_version_gate() -> void:
	...
	net_manager.disconnect_from_game()
	_expect(NetConstants.PROTOCOL_VERSION == 6, "The multiplayer protocol version must be 6.")
	...
	_expect(
		not bool(net_manager.call("_is_protocol_version_compatible", 5))
		...
		"NetManager must reject protocol versions 5/4/3/2 and legacy registrations with no version."
	)
