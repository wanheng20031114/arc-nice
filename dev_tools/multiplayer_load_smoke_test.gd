...
func _test_net_manager_protocol_version_gate() -> void:
	...
	net_manager.disconnect_from_game()
	_expect(NetConstants.PROTOCOL_VERSION == 84, "The multiplayer protocol version must be 84.")
	_expect(NetConstants.CHANNEL_COUNT == 8, "Protocol v84 must retain eight ENet channels.")
	_expect(
	...
	_expect(
		not bool(net_manager.call("_is_protocol_version_compatible", 83))
		...
		"Protocol v84 must reject v83 and all older or unversioned clients."
	)
...
func _test_net_manager_game_mode_authority() -> void:
	...
	_expect(
		...
		and int(NetManagerStore.GameMode.TEST_ARENA_P1E) == 8,
		"P1B/P1C/P1D/P1E must append wire values 5/6/7/8 without renumbering existing modes."
	)
	for mode_contract in [
		...
		[NetManagerStore.GameMode.TEST_ARENA_P1E, "test_arena_p1e", "测试场景 P1E"],
	]:
