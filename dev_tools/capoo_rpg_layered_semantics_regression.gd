...
const RUNTIME_SCENE := preload(
	"res://dev_tools/fixtures/capoo_rpg_layered_semantic_runtime.tscn"
)
const RPG_CONFIG := preload(
	"res://resources/config/enemies/capoo_rpg.tres"
)
const STONE_RPG_CONFIG := preload(
	"res://resources/config/enemies/stone_eroded_capoo_rpg.tres"
)
...
const SOURCE_NET_ID := 74_011
const SOURCE_OWNER_PEER_ID := 91
const TARGET_A_NET_ID := 74_012
const TARGET_B_NET_ID := 74_013
const ROLLBACK_TICK := 38
...
const GAMEPLAY_FIELDS: PackedStringArray = [
	...
	"last_rocket_spawn_x",
	"last_rocket_spawn_y",
	"last_rocket_direction_x",
	"last_rocket_direction_y",
	"last_rocket_lifetime",
	"last_rocket_target",
	"last_rocket_source_faction",
	"last_rocket_credit_peer",
	"last_rocket_instigator",
	"last_rocket_source_type",
...
func _run() -> void:
	...
	_expect(
		...
		"Every RPG policy coroutine must reach its completion sentinel."
	)
	...
	print("CAPOO_RPG_LAYERED_SEMANTICS_JSON %s" % JSON.stringify(result))
	if failures.is_empty():
		print("CAPOO_RPG_LAYERED_SEMANTICS_REGRESSION_OK")
		quit(0)
		return
...
func _verify_two_config_closure_and_capabilities() -> void:
	for config in [RPG_CONFIG, STONE_RPG_CONFIG]:
		var rpg := config.enemy_scene.instantiate() as CapooRPG
		_expect(
			rpg != null,
			"%s must instantiate the shared CapooRPG runner."
			% config.resource_path
		)
		if rpg == null:
			continue
		var script := rpg.get_script() as Script
		_expect(
		...
		_expect(
			...
			"%s must publish centralized/layered/dynamic/indexed RPG capability without inherited touch damage."
			% config.resource_path
		)
		rpg.free()
...
func _run_mode(simulation_mode: int) -> Dictionary:
	...
	var rocket_service := (
		...
		.get_capoo_rpg_rocket_simulation_service()
	)
	_expect(
		rocket_service != null and rocket_service.is_bound(),
		"%s authored runtime must bind the RPG data-rocket service." % mode_name
	)
	if rocket_service != null:
		rocket_service.process_mode = Node.PROCESS_MODE_DISABLED
		rocket_service.clear()
		_expect(
			rocket_service.reserve(8),
			"%s RPG rocket fixture must reserve deterministic capacity." % mode_name
		)

	var source: Variant = runtime.get_node("EnemyContainer/RPGSource")
	...
	var source_config := RPG_CONFIG.duplicate(true) as CapooRPGConfig
	...
	_reset_source_after_bootstrap(source, source_config, target_a, rocket_service)
	...
	if simulation_mode == POLICY.Mode.LAYERED_CONTACT:
		_expect(
			...
			"LAYERED_CONTACT must replace RPG TouchDamageArea with indexed contact."
		)
		...
		_expect(
			...
			"RPG CONTACT admission must not disable a separate attack Area."
		)
	else:
		_expect(
...
func _reset_source_after_bootstrap(
	source: Variant,
	source_config: CapooRPGConfig,
	target_a: Enemy,
	rocket_service: Variant
) -> void:
	if rocket_service != null:
		rocket_service.clear()
		rocket_service.reserve(8)
	...
	source.combat_state = CapooRPG.CombatState.CHASE
	...
	source.layered_rpg_event_consumes_tick = false
	source.layered_rpg_windup_ready_to_fire = false
...
func _capture_snapshot(
	...
	var last_rocket: Dictionary = (
		source.rocket_records[-1]
		if not source.rocket_records.is_empty()
		else {}
	)
	return {
		"tick": tick_index,
		...
		"muzzle_progress": _quantize(source.last_muzzle_heat_progress),
		"muzzle_visible": 1 if source.muzzle_heat.visible else 0,
		"muzzle_position_x": _quantize(source.muzzle_heat.position.x),
		"muzzle_position_y": _quantize(source.muzzle_heat.position.y),
		"muzzle_rotation": _quantize(source.muzzle_heat.rotation),
		"muzzle_scale": _quantize(source.muzzle_heat.scale.x),
		"muzzle_green": _quantize(source.muzzle_heat.color.g),
		"muzzle_alpha": _quantize(source.muzzle_heat.color.a),
		...
		"rocket_attempts": source.rocket_attempt_count,
		"rocket_count": source.rocket_records.size(),
		"rocket_service_live": (
			rocket_service.get_live_count() if rocket_service != null else -1
		),
		"legacy_rocket_nodes": _count_legacy_rocket_nodes(runtime),
		"last_rocket_spawn_x": int(last_rocket.get("spawn_x", 0)),
		"last_rocket_spawn_y": int(last_rocket.get("spawn_y", 0)),
		"last_rocket_direction_x": int(last_rocket.get("direction_x", 0)),
		"last_rocket_direction_y": int(last_rocket.get("direction_y", 0)),
		"last_rocket_lifetime": int(last_rocket.get("remaining_lifetime", 0)),
		"last_rocket_target": int(last_rocket.get("target_id", 0)),
		"last_rocket_source_faction": int(
			last_rocket.get("source_faction", -1)
		),
		"last_rocket_credit_peer": int(last_rocket.get("credit_peer_id", 0)),
		"last_rocket_instigator": int(last_rocket.get("instigator_id", 0)),
		"last_rocket_source_type": String(last_rocket.get("source_type", "")),
...
func _capture_rollback_state(source: Variant) -> Dictionary:
	return {
		...
		"muzzle": source.last_muzzle_heat_progress,
	}
...
func _validate_mode_invariants(
	...
	if snapshots.size() != TEST_TICKS:
		failures.append("%s must capture every RPG tick." % mode_name)
		return
	_expect_state(snapshots, 1, CapooRPG.CombatState.WINDUP, mode_name)
	_expect(
		int(snapshots[1]["fire_direction_y"]) > 0
		and int(snapshots[1]["windup_left"]) > 0,
		"%s committed RPG windup must follow target motion despite leaving range."
		% mode_name
	)
	_expect_state(snapshots, 4, CapooRPG.CombatState.FIRE, mode_name)
	_expect(
		int(snapshots[3]["rocket_count"]) == 1
		and int(snapshots[3]["rocket_service_live"]) == 1
		and int(snapshots[3]["legacy_rocket_nodes"]) == 0,
		"%s first FIRE must spawn exactly one data rocket and no legacy node."
		% mode_name
	)
	_expect_state(snapshots, 5, CapooRPG.CombatState.CHASE, mode_name)
	_expect(
		int(snapshots[4]["movement_submissions"])
		> int(snapshots[3]["movement_submissions"]),
		"%s invalid FIRE target must cancel and resume CHASE motion on tick 5."
		% mode_name
	)
	for tick_number in range(6, 25):
		_expect(
			int(snapshots[tick_number - 1]["movement_submissions"])
			> int(snapshots[tick_number - 2]["movement_submissions"]),
			"%s cooldown/range/LOS CHASE must keep moving on tick %d."
			% [mode_name, tick_number]
		)
	_expect_state(snapshots, 25, CapooRPG.CombatState.WINDUP, mode_name)
	_expect_state(snapshots, 26, CapooRPG.CombatState.CHASE, mode_name)
	_expect(
		int(snapshots[25]["movement_submissions"])
		> int(snapshots[24]["movement_submissions"]),
		"%s friendly target cancellation must permit same-tick motion."
		% mode_name
	)
	_expect_state(snapshots, 27, CapooRPG.CombatState.WINDUP, mode_name)
	_expect_state(snapshots, 28, CapooRPG.CombatState.CHASE, mode_name)
	_expect(
		int(snapshots[27]["movement_submissions"])
		> int(snapshots[26]["movement_submissions"]),
		"%s dead target cancellation must permit same-tick motion."
		% mode_name
	)
	_expect_state(snapshots, 29, CapooRPG.CombatState.WINDUP, mode_name)
	_expect_state(snapshots, 32, CapooRPG.CombatState.CHASE, mode_name)
	_expect(
		int(snapshots[31]["movement_submissions"])
		== int(snapshots[30]["movement_submissions"])
		and int(snapshots[32]["movement_submissions"])
		> int(snapshots[31]["movement_submissions"]),
		"%s expiry LOS cancellation must consume tick 32; range failure moves tick 33."
		% mode_name
	)
	_expect_state(snapshots, 34, CapooRPG.CombatState.WINDUP, mode_name)
	_expect_state(snapshots, 37, CapooRPG.CombatState.FIRE, mode_name)
	_expect(
		int(snapshots[36]["action_sequence"]) == 7
		and int(snapshots[36]["rocket_count"]) == 2
		and int(snapshots[36]["rocket_service_live"]) == 2
		and String(snapshots[36]["action_log"])
		== (
			"1:windup|2:fire|3:windup|4:windup|"
			+ "5:windup|6:windup|7:fire"
		),
		"%s cancellations must preserve RPG action_sequence and data rocket count."
		% mode_name
	)
	_expect(
		int(snapshots[3]["last_rocket_direction_x"]) == 0
		and int(snapshots[3]["last_rocket_direction_y"]) == 1_000_000
		and int(snapshots[36]["last_rocket_direction_x"]) == -1_000_000
		and int(snapshots[36]["last_rocket_direction_y"]) == 0,
		"%s RPG rockets must freeze the last committed moving-target direction."
		% mode_name
	)
	_expect(
		int(snapshots[3]["last_rocket_target"]) == TARGET_A_NET_ID
		and int(snapshots[3]["last_rocket_source_faction"])
		== CombatRelationService.HOSTILE_WAVE
		and int(snapshots[36]["last_rocket_target"]) == TARGET_B_NET_ID
		and int(snapshots[36]["last_rocket_source_faction"])
		== CombatRelationService.PLAYER_ALLIED
		and int(snapshots[36]["last_rocket_credit_peer"])
		== SOURCE_OWNER_PEER_ID
		and int(snapshots[36]["last_rocket_instigator"]) == SOURCE_NET_ID
		and String(snapshots[36]["last_rocket_source_type"])
		== "capoo_rpg_rocket"
		and int(snapshots[36]["last_rocket_lifetime"])
		== _quantize(RPG_CONFIG.projectile_lifetime),
		"%s data rockets must freeze target, faction and reward-credit attribution."
		% mode_name
	)
	_expect(
		...
		"%s rollback must restore individual physics without changing RPG state/RNG."
		% mode_name
	)
	_expect(
		...
		"%s rollback must transfer RPG ownership exactly at tick %d."
	...
	if simulation_mode == POLICY.Mode.LAYERED_CONTACT:
		_expect(
			...
			"CONTACT rollback must release indexed authority and restore RPG Area."
		)
	var expected_phase := (
		&"decision"
	...
	var source: Variant = context["source"]
	_expect(
		source.windup_start_phases.size() == 5
		...
		"%s RPG attack commits must occur only in the expected %s phase."
	...
	_expect_state(snapshots, 49, CapooRPG.CombatState.CHASE, mode_name)
	_expect(
		int(snapshots[48]["rocket_count"]) == 2,
		"%s runtime-friendly target must not acquire a third attack." % mode_name
	)
...
func _compare_mode_traces(runs: Dictionary) -> void:
	...
	for comparison_mode in [
		...
		if comparison_snapshots.size() != legacy_snapshots.size():
			failures.append("%s RPG trace length differs from LEGACY." % mode_name)
			continue
		...
		for tick_index in range(legacy_snapshots.size()):
			...
			for field_name in GAMEPLAY_FIELDS:
				if legacy_snapshot.get(field_name) == comparison_snapshot.get(field_name):
					continue
				failures.append(
					"%s RPG diverged from LEGACY at tick %d field %s: legacy=%s comparison=%s"
					% [
						mode_name,
						tick_index + 1,
						field_name,
...
func _expect_state(
	...
	_expect(
		...
		"%s RPG tick %d must be state %d (got %s)."
		% [
			mode_name,
			tick_number,
			expected_state,
...
func _count_legacy_rocket_nodes(root_node: Node) -> int:
	var result := 1 if root_node is CapooRPGRocket else 0
	for child in root_node.get_children():
		result += _count_legacy_rocket_nodes(child)
	return result
