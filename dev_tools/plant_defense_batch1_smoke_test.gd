...
func _test_config_and_scene_contracts() -> void:
	...
	var planting_base_config := PlantDefenseRegistry.get_config(&"planting_base")
	_expect(
	...
	var life_tower_config := PlantDefenseRegistry.get_config(
		&"life_tower"
	) as LifeTowerConfig
	_expect(
	...
	_expect(
		life_tower_config.max_health == 2400
		and life_tower_config.physical_defense == 5
		and life_tower_config.magic_defense == 0
		and life_tower_config.footprint_size == Vector2i(2, 2)
		and life_tower_config.building_category
		== PlantDefenseConfig.BuildingCategory.SUPPORT_TOWER
		and life_tower_config.placement_surface
		== PlantDefenseConfig.PlacementSurface.GRASS_ONLY
	...
	_expect(is_equal_approx(agave_config.attack_range, 130.0), "龙舌兰索敌半径必须为130。")
	_expect(
	...
	if agave != null:
		...
		_expect(target_circle != null and is_equal_approx(target_circle.radius, 130.0), "索敌Area半径必须为130。")
...
func _test_realtime_selection_and_cancel() -> void:
	...
	for card in controller.selection_hud.cards:
		if card.plant_config == agave_config:
			agave_card = card
		elif card.plant_config == bamboo_mortar_config:
			bamboo_mortar_card = card
