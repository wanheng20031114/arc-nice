...
func _run() -> void:
	...
	test_root.name = "FireEnemySmokeTest"
	...
	if failures.is_empty():
		print("FIRE_ENEMY_SMOKE_TEST_OK")
		quit()
		return
...
func _test_resource_contract() -> void:
	...
	_expect(
		game_script_source.contains("res://resources/config/enemy_fire_ranged.tres"),
		"Game random spawn list does not contain the fire enemy config."
	)

	var projectile := FIRE_PROJECTILE_SCENE.instantiate() as EnemyFireProjectile
	_expect(projectile != null, "Projectile scene did not instantiate EnemyFireProjectile.")
	if projectile != null:
...
func _spawn_projectile(position: Vector2, direction: Vector2) -> EnemyFireProjectile:
	var projectile := FIRE_PROJECTILE_SCENE.instantiate() as EnemyFireProjectile
	projectile.setup(direction, 1, 190.0, 2.0)
...
func _wait_for_projectile(max_frames: int) -> EnemyFireProjectile:
	for _frame_index in range(max_frames):
		await physics_frame
		for child in test_root.get_children():
			var projectile := child as EnemyFireProjectile
			if projectile != null:
				return projectile
	return null
...
func _get_projectile_ids() -> Array[int]:
	...
	for child in test_root.get_children():
		if child is EnemyFireProjectile:
