...
func _test_skill3_config() -> void:
	...
	_expect(is_equal_approx(SKILL3_CONFIG.orb_grow_delay_max, 2.9), "Skill3 grow delay max mismatch.")
	...
	for _index in range(32):
		...
		_expect(grow_delay >= 2.2 and grow_delay <= 2.9, "Skill3 grow delay must stay inside configured range.")
...
func _test_boss_skill3_schedule() -> void:
	...
	for record in host.projectile_records:
		...
		_expect(is_equal_approx(float(record.get("speed", 0.0)), 70.0), "Skill3 registered wrong orb speed.")
		_expect(float(record.get("lifetime", 0.0)) >= 2.2 and float(record.get("lifetime", 0.0)) <= 2.9, "Skill3 registered grow delay outside range.")
...
func _test_multiplayer_projectile_instantiation() -> void:
	...
	if projectile != null:
		...
		_expect(is_equal_approx(projectile.speed, 70.0), "Registry Skill3 orb speed mismatch.")
