...
const CODEX_ENTRY := preload(
	"res://resources/config/encyclopedia/enemies/combat_robot_shield_bearer_elite.tres"
)
...
func _run() -> void:
	...
	_test_protocol_attack_registry_and_pool_absence()
	_test_p1b_contract()
	_test_zero_direct_references()
	for _cleanup_frame in range(3):
		await process_frame
	if failures.is_empty():
		print("COMBAT_ROBOT_SHIELD_BEARER_ELITE_INTEGRATION_SMOKE_TEST_OK")
		quit(0)
		return
...
func _test_codex_and_fate() -> void:
	_expect(
		CODEX_ENTRY.entry_id == &"combat_robot_shield_bearer_elite"
		...
		and CODEX_ENTRY.sort_order == 555
		and CODEX_ENTRY.traits
		== PackedStringArray(["精英", "50次格挡", "紫能塔盾"])
		and CODEX_ENTRY.description.contains("第50发")
		and CODEX_ENTRY.description.contains("第51发")
		and CODEX_ENTRY.description.contains("侧后方"),
		"精英盾兵图鉴必须保持排序555、机械生命精英与50→51边界说明。"
	)
	...
	_expect(
		...
		"图鉴计数必须为64/51/12/1、机械生命11、人工造物4，且精英盾兵仅注册一次。"
	)
...
func _test_protocol_attack_registry_and_pool_absence() -> void:
	_expect(NET_CONSTANTS.PROTOCOL_VERSION == 70, "协议v70必须隔离地下教会10/20/40敌人构成与同时存活上限15的正式作战合同，并保留v69植被科研、精英盾兵与既有资源语义。")
	_expect(
	...
	for source_path in [
		...
		_expect(
			not source_text.contains("combat_robot_shield_bearer_elite"),
			"精英盾兵不得注册投射物类型、对象池或投射物预加载：%s。"
			% source_path
		)
...
func _test_p1b_contract() -> void:
	for wave_path in P1B_WAVE_PATHS:
		...
		_expect(
			...
			"P1B必须保持五种普通机器人各200台、总数1000且零精英盾兵直引。"
		)
		var entries_line := ""
...
func _test_zero_direct_references() -> void:
	for label: String in ZERO_DIRECT_REFERENCE_ROOTS:
		...
		_expect(
			references.is_empty(),
			"%s不得直接引用精英举盾战斗机器人：%s。" % [label, references]
		)
