...
func _run() -> void:
	...
	print(
		(
			"FIRE_SORCERER_ELITE_PERFORMANCE enemies=%d steady_volleys=%d "
	...
	_expect(
		...
		"300 synchronized Elite Fire Sorcerers must keep isolated p95 below 16.67 ms."
	)
	_expect(
	...
	_expect(
		...
		"Every Elite Fire Sorcerer must complete all three measured attack rounds."
	)
	_expect(
...
func _measure_synchronized_300_enemy_cohort() -> Dictionary:
	...
	_expect(
		...
		"300 Elite Fire Sorcerers must cover every deterministic first-attack bucket."
	)
	...
	_expect(
		...
		"Elastic Elite Fire Sorcerer projectile leases must never be dropped."
	)
