...
func _run() -> void:
	if OS.get_cmdline_user_args().has(PLAYER_SNAPSHOT_ONLY_ARG):
		...
		if failures.is_empty():
			print("PROTOCOL_V60_PLAYER_SNAPSHOT_SMOKE_TEST_OK")
			quit()
			return
	...
	if failures.is_empty():
		print("PROTOCOL_V60_SNAPSHOT_SMOKE_TEST_OK")
		quit()
		return
	for failure in failures:
		push_error(failure)
...
func _test_channel_contract() -> void:
	_expect(NetConstants.PROTOCOL_VERSION == 60, "Protocol must be v60.")
	_expect(
		Enemy.NETWORK_VISUAL_STATUS_MASK == 0x7f,
		"Protocol v60 must retain the scene-specific v45 bits 5..6 for shield stages and ninja boost."
	)
	_expect(
	...
	_expect(
		...
		"Protocol v60 channel assignments must remain stable."
	)
