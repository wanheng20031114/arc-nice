...
func _run() -> void:
	...
	if failures.is_empty():
		print("PROTOCOL_V47_SNAPSHOT_SMOKE_TEST_OK")
		quit()
		return
...
func _test_channel_contract() -> void:
	_expect(NetConstants.PROTOCOL_VERSION == 47, "Protocol must be v47.")
	_expect(
		Enemy.NETWORK_VISUAL_STATUS_MASK == 0x7f,
		"Protocol v47 must retain the scene-specific v45 bits 5..6 for shield stages and ninja boost."
	)
	_expect(
	...
	_expect(
		...
		"Protocol v47 channel assignments must remain stable."
	)
