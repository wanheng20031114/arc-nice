...
func _run() -> void:
	if OS.get_cmdline_user_args().has(PLAYER_SNAPSHOT_ONLY_ARG):
		...
		if failures.is_empty():
			print("PROTOCOL_V81_PLAYER_SNAPSHOT_SMOKE_TEST_OK")
			quit()
			return
	...
	if failures.is_empty():
		print("PROTOCOL_V81_SNAPSHOT_SMOKE_TEST_OK")
		quit()
		return
	for failure in failures:
		push_error(failure)
...
func _test_channel_contract() -> void:
	_expect(NetConstants.PROTOCOL_VERSION == 81, "Protocol must be v81.")
	_expect(
		Enemy.NETWORK_VISUAL_STATUS_MASK == 0x7f,
		"Protocol v81 must retain scene-specific bits 5..6 for shield stages, ninja boost, and main-battle airborne visuals."
	)
	_expect(
	...
	_expect(
		...
		"Protocol v81 channel assignments must remain stable."
	)
