...
func _run() -> void:
	...
	if failures.is_empty():
		print("PROTOCOL_V21_SNAPSHOT_SMOKE_TEST_OK")
		quit()
		return
...
func _test_channel_contract() -> void:
	_expect(NetConstants.PROTOCOL_VERSION == 21, "Protocol must be v21.")
	...
	_expect(
		...
		"Protocol v21 channel assignments must remain stable."
	)
...
func _test_enemy_codec_reuse_and_packet_budget() -> void:
	...
	_expect(
		keyframe.size() == 1106,
		"A v21 46-enemy keyframe must use 1106 bytes and stay within the 1200-byte budget."
	)
