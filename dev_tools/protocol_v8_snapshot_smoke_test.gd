...
func _run() -> void:
	if OS.get_cmdline_user_args().has(PLAYER_SNAPSHOT_ONLY_ARG):
		...
		if failures.is_empty():
			print("PROTOCOL_V79_PLAYER_SNAPSHOT_SMOKE_TEST_OK")
			quit()
			return
	...
	if failures.is_empty():
		print("PROTOCOL_V79_SNAPSHOT_SMOKE_TEST_OK")
		quit()
		return
	for failure in failures:
		push_error(failure)
...
func _test_channel_contract() -> void:
	_expect(NetConstants.PROTOCOL_VERSION == 79, "Protocol must be v79.")
	_expect(
		Enemy.NETWORK_VISUAL_STATUS_MASK == 0x7f,
		"Protocol v79 must retain scene-specific bits 5..6 for shield stages, ninja boost, and main-battle airborne visuals."
	)
	_expect(
	...
	_expect(
		...
		"Protocol v79 channel assignments must remain stable."
	)
