...
func _test_channel_contract() -> void:
	_expect(NetConstants.PROTOCOL_VERSION == 17, "Protocol must be v17.")
	...
	_expect(
		...
		"Protocol v17 channel assignments must remain stable."
	)
...
func _test_enemy_codec_reuse_and_packet_budget() -> void:
	...
	_expect(
		keyframe.size() == 1122,
		"A v17 56-enemy keyframe must use 1122 bytes and stay within the 1200-byte budget."
	)
