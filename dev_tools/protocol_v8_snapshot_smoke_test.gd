...
func _test_channel_contract() -> void:
	_expect(NetConstants.PROTOCOL_VERSION == 57, "Protocol must be v57.")
	_expect(
		Enemy.NETWORK_VISUAL_STATUS_MASK == 0x7f,
		"Protocol v57 must retain the scene-specific v45 bits 5..6 for shield stages and ninja boost."
	)
	_expect(
	...
	_expect(
		...
		"Protocol v57 channel assignments must remain stable."
	)
