...
const P1D_SCENE_PATH := "res://scene/game_modes/tower_defense/test_arenas/test_grass_arena_p1d.tscn"
...
func _test_lightweight_manifest(coordinator: Node) -> void:
	...
	for tower_contract in [
		...
		[P1D_SCENE_PATH, P1D_CAMPAIGN_PATH],
		...
		_expect(
			...
			"P1A/P1B/P1C/P1D/P2必须加载各自战役、所选角色与塔防运行时。"
		)
...
func _test_main_menu_selector(coordinator: Node) -> void:
	...
	_expect(
		...
		and entry_subtitle.text.contains("P1A / P1B / P1C / P1D / P2 / P3")
	...
	for tower_entry in [
		...
		[
			TestArenaChoiceOverlay.ARENA_P1D_ID,
			P1D_SCENE_PATH,
			TestArenaChoiceOverlay.P1D_TAB_INDEX,
			"P1D",
		],
		[
		...
		_expect(
			...
			== str(tower_entry[1]),
			"P1A/P1B/P1C/P1D/P2必须进入角色选择并保留各自场景路径。"
		)
		character_selector.close()
		await process_frame
		selector.close()
