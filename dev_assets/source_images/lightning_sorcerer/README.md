...
图集。2026-07-30 将移动从四帧重做为独立的八相位步行循环，蓄力、施法和死亡
仍保留主图中的四帧动画。

## 文件

- `lightning_sorcerer_move_8pose_v1_imagegen_source.png`：4×2、八相位移动的
  绿色背景最终源图。
- `lightning_sorcerer_move_8pose_v1_alpha.png`：对应的透明最终源图。
...
旧的 `lightning_sorcerer_move_{imagegen_source,alpha}.png` 保留为历史四帧稿，
不再参与当前移动运行时构建。
...
随后是左脚对应的接触、下沉、通过、抬升。其余三个动作仍是单行四帧。共同约束
如下：
...
40×40。构建同时验证横向质心峰峰值不超过 1 像素。蓄力、施法和死亡仍各用
四帧并保留在 `resources/texture/lightning_sorcerer.png` 主图中。全部运行时输出
均保持二值 alpha，透明像素 RGB 为零。
