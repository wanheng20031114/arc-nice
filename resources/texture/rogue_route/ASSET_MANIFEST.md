...
- `hud/hud_core_life_v5.png`：塔防核心生命；20×20 逻辑画布上的蓝心，经最近邻精确放大两倍
- `hud/hud_action_points_v4.png`：行动力；单只青蓝矿工靴
- `hud/hud_light_stone_v1.png`：全队共享的光石
- `../xirang_icon.png`：复用已有息壤水晶图标

核心生命与行动力为 40×40 透明 PNG；新核心生命图标的主体严格占 17×16 个逻辑像素，与行动力图标的视觉密度一致。光石为原生 32×32，息壤为原生 37×37。顶部 HUD 统一给它们 40px 对齐格，但使用 `STRETCH_KEEP_CENTERED` 与最近邻过滤，绝不把小图做非整数放大；均未使用平滑插值。
...
每类节点的 `RogueRouteNodeTypeConfig.icon` 是运行时唯一图标真源；地图节点与作战简报均读取同一配置，不再各自维护图标路径。新增或替换节点美术时只需更新对应类型资源。
...
2026-08-08 结构审查进一步移除了根目录内已被 V3/V4 成品替代、且无运行时引用的旧节点图、旧空节点、旧连接/地面图块、旧 HUD 面板与 seed 图标。`nodes/`、`links/`、`inventory/`、`hud/` 现为路线运行时素材的正式分类入口；清单中标注的中性环、交汇件、端点件等模块化复用素材即使暂未直接引用也继续保留。
