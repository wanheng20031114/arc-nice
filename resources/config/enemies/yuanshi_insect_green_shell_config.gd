...
# 粒子从敌人中心附近生成的随机半径。
@export_range(0.0, 64.0, 0.5, "or_greater") var aura_particle_emission_radius: float = 3.0
# 粒子向四周喷出的最小速度。
@export_range(0.0, 1000.0, 0.5, "or_greater") var aura_particle_speed_min: float = 20.0
# 粒子向四周喷出的最大速度。
@export_range(0.0, 1000.0, 0.5, "or_greater") var aura_particle_speed_max: float = 32.0
# 粒子的最小尺寸倍率。
...
# 粒子的整体颜色和透明度。
@export var aura_particle_color: Color = Color(0.22, 0.95, 0.12, 0.88)

@export_group("翠壳范围提示")
# 伤害范围内部的半透明填充颜色。
@export var aura_fill_color: Color = Color(0.12, 0.72, 0.08, 0.08)
# 伤害范围边缘轮廓的颜色。
@export var aura_outline_color: Color = Color(0.38, 1.0, 0.2, 0.68)
# 伤害范围边缘轮廓的宽度。
@export_range(0.25, 8.0, 0.25, "or_greater") var aura_outline_width: float = 1.25
