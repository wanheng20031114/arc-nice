...
3. 简易制作只读写发起玩家背包；23条正式建筑生产配方的产物统一进入共享仓库。
...
结算规则分为两类：简易制作在缺料或背包容量不足时整笔拒绝，不扣除任何投入；共享生产在启动与结算前原子预检，缺料、缺少可用仓库或仓库已满时暂停等待。水源采集器与挖土装置虽然不消耗仓库原料，产物仍必须在完成瞬间原子写入共享仓库，不会暂存在建筑或玩家背包中。
...
域缩写：`背→背`为个人背包原子事务，`仓→仓`为共享仓库原子事务，`环境→仓`为环境输入后直存共享仓库，`无→仓`为无材料投入并直存共享仓库。
