...
Player meta 从 38 增加到 42 bytes，即每个快照实体增加 4 bytes。Enemy 同样使用显式 `health_revision`：Host 的权威生命变更、可靠反馈与实时快照共用敌人节点上的递增修订号，Client 只接受比当前代理更新的生命状态。绿色史莱姆现已取消恢复能力，当前敌人在同一 net-id 生命周期内通常只会损失生命；显式修订号仍作为跨通道乱序屏障，并为未来合法的生命上升保留协议能力。完整敌人 keyframe 记录因此为 24 bytes；每包上限调整为 46 条，连同 `uint16` 计数共 1106 bytes，仍低于 1200-byte 应用层预算。
